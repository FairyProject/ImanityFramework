...
import org.imanity.framework.jongo.query.QueryFactory;
import org.imanity.framework.jongo.bson.BsonDocument;

import java.util.ArrayList;
import java.util.List;

public class Insert { // Imanity - public
    ...
    Insert(DBCollection collection, WriteConcern writeConcern, Serializer serializer, ObjectIdUpdater objectIdUpdater, QueryFactory queryFactory) {
    ...
    private DBObject convertToDBObject(Object pojo, Object id) {
        BsonDocument document = asBsonDocument(serializer, pojo);
        return new LazyIdDBObject(document.toByteArray(), serializer, id);
    }

    public final static class LazyIdDBObject extends LazyDBObject { // Imanity - public

        private Object bsonId;
        private final Serializer serializer;

        public LazyIdDBObject(byte[] data, Serializer serializer, Object _id) {
        ...
        private Object asBsonId(Object _id) {
            ...
            return asBsonDocument(serializer, _id).toDBObject();
        }
    ...
    private static BsonDocument asBsonDocument(Serializer serializer, Object obj) {
        try {
            return serializer.serialize(obj);
        } catch (Exception e) {
            String message = String.format("Unable to save object %s due to a serializing error", obj);
