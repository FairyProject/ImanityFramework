...
import static org.imanity.framework.jongo.ResultHandlerFactory.newResultHandler;
...
public class FindOne {
    ...
    FindOne(DBCollection collection, ReadPreference readPreference, Deserializer deserializer, QueryFactory queryFactory, String query, Object... parameters) {
    ...
    // Imanity start - example query
    FindOne(DBCollection collection, ReadPreference readPreference, Deserializer deserializer, QueryFactory queryFactory, ObjectIdUpdater idUpdater, Object query) {
        this.deserializer = deserializer;
        this.collection = collection;
        this.readPreference = readPreference;
        this.queryFactory = queryFactory;
        this.query = this.queryFactory.createQuery(idUpdater, query);
    }
    // Imanity end

    public <T> T as(final Class<T> clazz) {
        return map(newResultHandler(clazz, deserializer));
    }
