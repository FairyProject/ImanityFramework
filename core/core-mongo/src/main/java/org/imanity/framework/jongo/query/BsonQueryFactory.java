...
package org.imanity.framework.jongo.query;
...
import org.imanity.framework.jongo.Insert;
import org.imanity.framework.jongo.ObjectIdUpdater;
...
public class BsonQueryFactory implements QueryFactory {
    ...
    public BsonQueryFactory(Serializer serializer) {
        this(serializer, DEFAULT_TOKEN);
    }

    public BsonQueryFactory(Serializer serializer, String token) {
        this.token = token;
        this.serializer = serializer;
    }

    @Override
    public Query createQuery(ObjectIdUpdater idUpdater, Object example) {
        Object id = idUpdater.getId(example);
        BsonDocument document;
        try {
            document = this.serializer.serialize(example);
        } catch (Exception e) {
            String message = String.format("Unable to save object %s due to a serializing error", example);
            throw new IllegalArgumentException(message, e);
        }

        if (id == null) {
            return new BsonQuery(document.toDBObject());
        }
        return new BsonQuery(new Insert.LazyIdDBObject(document.toByteArray(), this.serializer, id));
    }
    ...
    private Object marshallDocument(Object parameter) {
        ...
        if (parameter instanceof Enum) {
            ...
            BsonDocument document = serializer.serialize(parameter);
    ...
    private Object marshallParameterAsPrimitive(Object parameter) {
        ...
        BsonDocument document = serializer.serialize(primitiveWrapper);
